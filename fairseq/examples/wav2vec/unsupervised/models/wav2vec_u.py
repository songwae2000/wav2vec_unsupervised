...
# ---------------------------------------------------------------------------
# Segmentation types and configs
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Main Wav2vec_U model
# ---------------------------------------------------------------------------

@register_model("wav2vec_u", dataclass=Wav2vec_UConfig)
class Wav2vec_U(BaseFairseqModel):
    """
    Wav2Vec Unsupervised (Wav2Vec-U) model.

    Implements unsupervised speech recognition using a GAN framework:
    - The Generator maps clustered audio features to phoneme-like distributions.
    - The RealData provider supplies one-hot phoneme sequences from text.
    - The Discriminator distinguishes real phoneme sequences from generated ones.

    Training alternates between discriminator and generator update steps.
    """
    ...
    def __init__(self, cfg: Wav2vec_UConfig, target_dict):
        ...
        # --- Separation of Concerns: three distinct components ---

        # 1. Discriminator: classifies real vs fake samples
        self.discriminator = Discriminator(output_size, cfg)
        for p in self.discriminator.parameters():
            p.param_group = "discriminator"

        # 2. Generator: produces fake samples from audio features
        d = cfg.input_dim
        ...
        # 3. RealData: provides real samples (one-hot phoneme sequences)
    ...
    def discrim_step(self, num_updates):
        """Returns True on odd update steps (discriminator's turn)."""
        return num_updates % 2 == 1

    def get_groups_for_update(self, num_updates):
        """Return which parameter group to update on this step."""
    ...
    def normalize(self, dense_x):
        """Apply softmax/gumbel normalization and compute perplexity metrics."""
    ...
    def forward(
        ...
        # --- RealData provides "real" samples ---
        token_x = self.real_data.get_samples(random_label, dense_x.device)
        token_padding_mask = self.real_data.get_padding_mask(random_label)

        # --- Discriminator judges both real and fake ---
        ...
        if d_step:
            # Discriminator step: real samples should be classified as real (0),
            # fake samples should be classified as fake (1)
